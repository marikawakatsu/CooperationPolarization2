...
const pvals = repeat([0.8, 0.85, 0.9, 0.95, 0.99], 10) # repeat([0.0, 0.25, 0.5, 0.75, 1.0], 10) # uniform populations for now
const us    = [0.001]
...
total_summary = SharedArray{Float64, 2}(length(pvals)*length(us)*length(vs),217) # SharedArray{Real}[] 

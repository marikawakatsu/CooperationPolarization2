...
const K = 3 # number of issues to care about

const pvals = repeat([0.0, 1.0], 10) # uniform populations for now
const βs    = [0.0001, 0.001, 0.01]
const vs    = [0.001, 0.025]
const ϵ     = 1.
const parameters = collect(Iterators.product(pvals,βs,vs))
...
total_summary = SharedArray{Float64, 2}(length(pvals)*length(βs)*length(vs),107) # SharedArray{Real}[] 

@time @sync @distributed for i in 1:length(pvals)*length(βs)*length(vs)
    ...
    open("data/gens_$(generations)/$(testtype)_betasweep_M$(M)K$(K)_$(start_time).csv", "w") do f
        write(f, "$(colnames)\n")

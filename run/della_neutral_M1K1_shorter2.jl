...
const us    = [0.001]
const vs    = [10^((log10(0.001)+log10(0.005))/2), 10^((log10(0.005)+log10(0.025))/2), 10^((log10(0.025)+log10(0.125))/2)]
const ϵ     = 1.
...
total_summary = SharedArray{Float64, 2}(length(pvals)*length(us)*length(vs),23) # SharedArray{Real}[] 

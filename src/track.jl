...
function make_summary_table( tracker::Tracker, gen_cutoff::Int64, generations::Int64 )
    ...
    short_strat_dist         = tracker.ens_strat_dist[floor(Int64,gen_cutoff):generations,:]
    short_pop_dist           = tracker.ens_pop_dist[floor(Int64,gen_cutoff):generations,:]
    short_opn_dist           = tracker.ens_opn_dist[floor(Int64,gen_cutoff):generations,:]
    short_total_means        = tracker.ens_total_means[floor(Int64,gen_cutoff):generations]
    short_total_vars         = tracker.ens_total_vars[floor(Int64,gen_cutoff):generations]
    short_total_cooperation  = tracker.ens_total_cooperation[floor(Int64,gen_cutoff):generations]
    short_party_cooperation  = tracker.ens_party_cooperation[floor(Int64,gen_cutoff):generations]
    short_enemy_cooperation  = tracker.ens_enemy_cooperation[floor(Int64,gen_cutoff):generations]
    short_opn_simpson        = tracker.ens_opn_simpson[floor(Int64,gen_cutoff):generations]
    short_sets_simpson       = tracker.ens_sets_simpson[floor(Int64,gen_cutoff):generations]
    short_cityblock_ind      = tracker.ens_cityblock_ind[floor(Int64,gen_cutoff):generations]
    short_cityblock_party    = tracker.ens_cityblock_party[floor(Int64,gen_cutoff):generations]
    short_cityblock_enemy    = tracker.ens_cityblock_enemy[floor(Int64,gen_cutoff):generations]
    short_hamming_ind        = tracker.ens_hamming_ind[floor(Int64,gen_cutoff):generations]
    short_hamming_party      = tracker.ens_hamming_party[floor(Int64,gen_cutoff):generations]
    short_hamming_enemy      = tracker.ens_hamming_enemy[floor(Int64,gen_cutoff):generations]
    ...
    ens_summary = hcat( generations, 
                        ...
                        tracker.pop.game.u, tracker.pop.game.v, tracker.pop.game.q, tracker.pop.game.gamma,
    ...
    col_list = hcat("generations", "N", "M", "K", "P", "b", "c", "β", "u", "v", "q", "gamma",
...
# function track_cityblock_ind!( tracker::Tracker, g::Int64 )
#     # track average pairwise cityblock distance in opinions (h)
#     # from Distance.jl (https://github.com/JuliaStats/Distances.jl): cityblock(x, y) = sum(abs(x - y))
#     # pairwise computes a symmetric N-by-N matrix of pairwise distances; diagonals are zero
#     # compute sum, divide by 2 to remove double counting, and divide by the number of pairs
#     tracker.pop.sim.cityblock_ind = pairwise(Cityblock(), tracker.pop.sets.h, dims=1)
#     tracker.ens_cityblock_ind[g]  = sum( tracker.pop.sim.cityblock_ind ) / 2 / tracker.pop.sets.n_pairs
# end

# function track_hamming_ind!( tracker::Tracker, g::Int64 )
#     # track average pairwise hamming distance in set memberships (h_bar)
#     # from Distance.jl (https://github.com/JuliaStats/Distances.jl): hamming(k, l) = sum(k .!= l)
#     # pairwise computes a symmetric N-by-N matrix of pairwise distances; diagonals are zero
#     # compute sum, divide by 2 to remove double counting, and divide by the number of pairs
#     tracker.pop.sim.hamming_ind = pairwise(Hamming(), tracker.pop.sets.h_bar, dims=1) 
#     tracker.ens_hamming_ind[g]  = sum( tracker.pop.sim.hamming_ind ) / 2 / tracker.pop.sets.n_pairs
# end

# function track_cityblock_party!( tracker::Tracker, g::Int64 )
#     # track average cityblock distance between individuals of the same party 
#     # pairwise computes a symmetric N/2-by-N/2 matrix of pairwise distances; diagonals are zero
#     # compute sum, divide by 2 to remove double counting, by 2 again to take average, and divide by the number of pairs per party
#     if tracker.pop.sets.P == 2
#         # tracker.pop.sim.cityblock_party .= 0 # reset # not needed because automatically overwritten
#         # track party 1 and party 2 separately
#         tracker.pop.sim.cityblock_party = pairwise(Cityblock(), tracker.pop.sets.h[1:tracker.pop.sets.N÷2,:], dims=1) # p=1
#         tracker.ens_cityblock_party[g] += sum( tracker.pop.sim.cityblock_party ) / 2
#         tracker.pop.sim.cityblock_party = pairwise(Cityblock(), tracker.pop.sets.h[tracker.pop.sets.N÷2+1:tracker.pop.sets.N,:], dims=1) # p=2
#         tracker.ens_cityblock_party[g] += sum( tracker.pop.sim.cityblock_party ) / 2

#         # take average 
#         tracker.ens_cityblock_party[g]  = tracker.ens_cityblock_party[g] / 2 / tracker.pop.sets.n_party_pairs
#     end
# end

# function track_hamming_party!( tracker::Tracker, g::Int64 )
#     # track average hamming distance between individuals of the same party 
#     # pairwise computes a symmetric N/2-by-N/2 matrix of pairwise distances; diagonals are zero
#     # compute sum, divide by 2 to remove double counting, by 2 again to take average, and divide by the number of pairs per party
#     if tracker.pop.sets.P == 2
#         # track party 1 and party 2 separately
#         tracker.pop.sim.hamming_party = pairwise(Hamming(), tracker.pop.sets.h_bar[1:tracker.pop.sets.N÷2,:], dims=1) # p=1
#         tracker.ens_hamming_party[g] += sum( tracker.pop.sim.hamming_party ) / 2
#         tracker.pop.sim.hamming_party = pairwise(Hamming(), tracker.pop.sets.h_bar[tracker.pop.sets.N÷2+1:tracker.pop.sets.N,:], dims=1) # p=2
#         tracker.ens_hamming_party[g] += sum( tracker.pop.sim.hamming_party ) / 2

#         # take average 
#         tracker.ens_hamming_party[g]  = tracker.ens_hamming_party[g] / 2 / tracker.pop.sets.n_party_pairs
#     end
# end